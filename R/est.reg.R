est.reg <-
function(crocmiss,col_indep){
cols<-ncol(crocmiss);rows<-nrow(crocmiss);estimated_matrix<-matrix(ncol=cols,nrow=rows)
deps<-c(1:(col_indep-1),(col_indep+1):cols); ndeps<-length(deps)
indep<-crocmiss[,col_indep]
rcoefs<-numeric()
for (i in 1:ndeps){
vari<-deps[i]
lm_fit<-lm(log10(crocmiss[,vari])~log10(indep)); lm_sum<-summary.lm(lm_fit)
...
indeps_lm<-lm(log10(newindep)~log10(crocmiss[,strongest]))
indeps_coef<-indeps_lm$coefficients
logestimate_indep<-indeps_coef[1]+indeps_coef[2]*log10(crocmiss[,strongest])
...
for (k in 1:ndeps){
vari<-deps[k]; variable<-crocmiss[,vari]
