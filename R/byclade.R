byclade <-
function(croc,remperc,ngroups,groups) {
newx1<-as.matrix(croc)
grouping<-as.factor(groups)
newx2<-as.matrix(croc)
totaldata<-nrow(croc)*ncol(croc)
...
splitgroups<-split(as.data.frame(croc),grouping)
